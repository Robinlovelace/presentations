...
The unconstrained 'gravity model' can then be written in its "multiplicative form" as:

$$
T_{ij} = k m_i^{\mu} n_j^{*} d_{ij} \beta
$$

where $k$ is a balancing factor, $m_i$ and $n_j$ are origin and destination masses respectively, $d_{ij}$ is the distance between $i$ and $j$
